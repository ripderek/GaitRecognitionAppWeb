...
#bandera para saber si se reproduce o no el streaming
streaming_enabled = True
#globalizar cap para poder reiniciar el video
cap = None 
#para indicar si se guardan las muestras en tiempo real
save_samples = True  # variable global
#para indicar si las muestras son para entrenamiento o prueba
train_sampele = True #si es True entonces son para entrenamiento, si es False son para prueba
...
#funciones para cambiar el valor de la bandera para iniciar o parar el streaming
def set_streaming(state: bool):
    """Cambiar estado de streaming (True = play, False = pause)"""
    global streaming_enabled
    streaming_enabled = state
...
def generate_frames(video_path,persona):
    global label,last_detections, selected_id,contador,vector_distancia_32_31,vector_distancia_28_27,vector_distancia_26_25,vector_distancia_31_23,vector_distancia_32_24,vector_distancia_16_12,vector_distancia_15_11,vector_distancia_32_16,vector_distancia_31_15,orientacionString,streaming_enabled,cap
    selected_id= None

    #1 primero registrar al participante, si no existe crearlo y devolver el id
    participanteID = sv.regitrarParticipante(persona)
    print(f"Participante {persona} ID: {participanteID}")
    #2 crear una muestra del participante y devolver el id de la muestra
    muestraid = sv.regitrarMuestra(participanteID)
    print(f"Muestra {muestraid} registrada para el participante {persona}")
    ...
    print(f"Video {videoID} registrado para la muestra {muestraid} del participante {persona}")
    ...
    while True:
                    ...
                    if xs is not None and ys is not None and len(xs) > 0 and len(ys) > 0:
                        ...
                        if pie_xd >= 380 or pie_xd_2>=380:
                            ...
                            if (contador>=35):
                                ...
                                print( f'promedio_32_31: {promedio_32_31}  / desviacion_32_31: {desviacion_32_31}')
                                print( f'promedio_28_27: {promedio_28_27}  /  desviacion_28_27: {desviacion_28_27}')                                
                                print( f'promedio_26_25: {promedio_26_25}  /  desviacion_26_25: {desviacion_26_25}')
                                print( f'promedio_31_23: {promedio_31_23}  /  desviacion_31_23: {desviacion_31_23}')
                                print( f'promedio_32_24: {promedio_32_24}  /  desviacion_32_24: {desviacion_32_24}')
                                print( f'promedio_16_12: {promedio_16_12}  /  desviacion_16_12: {desviacion_16_12}')
                                print( f'promedio_15_11: {promedio_15_11}  /  desviacion_15_11: {desviacion_15_11}')
                                print( f'promedio_32_16: {promedio_32_16}  /  desviacion_32_16: {desviacion_32_16}')
                                print( f'promedio_31_15: {promedio_31_15}  /  desviacion_31_15: {desviacion_31_15}')                                
        ...
        # ====================== SUPERPOSICIÓN DE NORMALIZACION ======================
        if 'normalizacion' in locals():  # verificar que se creó
            overlay_h, overlay_w = 200, 200  # tamaño deseado
            normalizacion_resized = cv2.resize(normalizacion, (overlay_w, overlay_h))
            h_frame, w_frame = frame_mejorado.shape[:2]
            x_offset = w_frame - overlay_w - 10
            y_offset = h_frame - overlay_h - 10

            # Para transparencia (opcional, comentar si quieres opaco)
            alpha = 0.7
            roi = frame_mejorado[y_offset:y_offset+overlay_h, x_offset:x_offset+overlay_w]
            cv2.addWeighted(normalizacion_resized, alpha, roi, 1 - alpha, 0, roi)
        # =========================================================================

        # Codificar frame_mejorado en JPEG para streaming
        ret, buffer = cv2.imencode('.jpg', frame_mejorado)
...
#funcion para reiniciar video
def restart_video(last_video_path):
    global cap
    if cap is not None and last_video_path is not None:
        cap.release()
        cap = cv2.VideoCapture(last_video_path)  # reabrir el último video
        set_selected_person(-1)


#para cambiar el estado de si guardar o no las muestras
def set_save_samples(value: bool):
    global save_samples
    save_samples = value


#para cambiar el estado si las muestras son para entrenamiento o prueba
def set_train_samples(value: bool):
    global train_sampele
    train_sampele = value